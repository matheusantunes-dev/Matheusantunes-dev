<p align="right">
  <a href="./README.en.md">🇺🇸 English Version</a>
</p>
<img src="1784727992877.png" alt="Matheus Antunes - Desenvolvedor">


<h1 align="center">
Olá, eu sou o Matheus Antunes Reis 👋
</h1>

<h3 align="center">
Software Developer • React • TypeScript • Kotlin • Android • Supabase
</h3>

<p align="center">
Construindo aplicações Web e Mobile focadas em performance, arquitetura limpa e soluções que resolvem problemas reais.
</p>

<p align="center">

<a href="https://www.linkedin.com/in/matheusantunes-dev/">
<img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="https://portifoliomatheusmr.netlify.app/">
<img src="https://img.shields.io/badge/Portfolio-121212?style=for-the-badge&logo=vercel&logoColor=white"/>
</a>

<a href="mailto:matheusantunesreis6@gmail.com">
<img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

<a href="https://wa.me/5538988064942">
<img src="https://img.shields.io/badge/WhatsApp-25D366?style=for-the-badge&logo=whatsapp&logoColor=white"/>
</a>

</p>

---

# Sobre mim

Sou Desenvolvedor de Software com foco em aplicações **Web** e **Mobile**.

Minha carreira começou na área fiscal, onde desenvolvi soluções para automatizar processos internos, reduzir erros operacionais e aumentar a produtividade. Essa experiência despertou meu interesse pelo desenvolvimento de software e hoje atuo construindo aplicações modernas utilizando React, TypeScript, Kotlin e Supabase.

Atualmente participo do desenvolvimento do **FotoVitrine**, aplicativo Android utilizado comercialmente, implementando funcionalidades como:

- Google Authentication
- Row Level Security (RLS)
- Supabase
- AsyncStorage
- APIs REST
- Recursos nativos Android
- Câmera personalizada
- Captura de fotos, vídeos e boomerangs

Gosto de transformar regras de negócio complexas em software simples, seguro e escalável.

---

# Stack

## Front-end

<p>

<img src="https://skillicons.dev/icons?i=html,css,js,ts,react,vite"/>

</p>

## Mobile

<p>

<img src="https://skillicons.dev/icons?i=kotlin,androidstudio"/>

</p>

## Backend / BaaS

<p>

<img src="https://skillicons.dev/icons?i=supabase,nodejs"/>

</p>

## Banco de Dados

<p>

<img src="https://skillicons.dev/icons?i=mysql,postgres"/>

</p>

## Ferramentas

<p>

<img src="https://skillicons.dev/icons?i=git,github,figma,vscode"/>

</p>

---

# Experiência

## Software Development Intern
**Loja Quatro Estações**

Atuação no desenvolvimento do aplicativo Android **FotoVitrine**.

### Principais responsabilidades

- Desenvolvimento Android utilizando Kotlin
- Integração com Supabase
- Implementação de Google Authentication
- Row Level Security (RLS)
- Persistência local com AsyncStorage
- Recursos nativos de câmera
- Versionamento utilizando Git/GitHub

---

## Auxiliar Fiscal

Durante minha atuação desenvolvi ferramentas internas que automatizaram processos fiscais, reduziram erros operacionais e otimizaram atividades repetitivas.

Essa experiência fortaleceu minha capacidade de compreender regras de negócio antes mesmo de escrever código.

---

# Projetos

### 📸 FotoVitrine

Aplicativo Android para produção de mídias comerciais.

**Stack**

- Kotlin
- Android SDK
- Supabase
- Google Auth
- RLS

---

### 🌱 PROCONSVATE

Sistema educacional voltado para conscientização ambiental.

---

### 🤖 Mail Sense AI

Aplicação para análise inteligente de e-mails utilizando IA.

---

# Atualmente estudando

- Clean Architecture
- Android Development
- Arquitetura de Software
- TypeScript Avançado
- Kotlin
- Boas práticas de Engenharia de Software

---

# GitHub Analytics

<p align="center">

<img height="170em" src="https://github-readme-stats.vercel.app/api?username=matheusantunes-dev&show_icons=true&theme=tokyonight&hide_border=true"/>

<img height="170em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=matheusantunes-dev&layout=compact&theme=tokyonight&hide_border=true"/>

</p>

<p align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=matheusantunes-dev&theme=tokyo-night"/>

</p>

---

> "Software é a ponte entre problemas reais e soluções escaláveis."
